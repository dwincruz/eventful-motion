    ...
    const setupMotion = async () => {
      ...
      context = gsap.context(() => {
        ...
        media.add({ desktop: "(min-width: 769px) and (prefers-reduced-motion: no-preference)", mobile: "(max-width: 768px) and (prefers-reduced-motion: no-preference)", reduce: "(prefers-reduced-motion: reduce)" }, (match) => {
          ...
          if (reduce) {
            gsap.set([".events-kicker", ".events-title", ".events-copy", ".filter", cards, cardImages], { clearProps: "all", opacity: 1 });
            return;
          }
          ...
          const directions = desktop
            ? [
                { x: -140, y: 0, rotation: -8 },
                { x: 0, y: 120, rotation: 6 },
                { x: 140, y: 0, rotation: 8 },
                { x: -110, y: 90, rotation: 7 },
                { x: 0, y: -100, rotation: -6 },
                { x: 110, y: 90, rotation: -7 },
              ]
            : [
                { x: -28, y: 52, rotation: -3 },
                { x: 28, y: 52, rotation: 3 },
                { x: -20, y: 44, rotation: -2 },
                { x: 20, y: 44, rotation: 2 },
                { x: -16, y: 38, rotation: -2 },
                { x: 16, y: 38, rotation: 2 },
              ];
          ...
          cards.forEach((card, index) => {
            ...
            gsap.fromTo(card, {
              ...
              scale: desktop ? .84 + (index % 3) * .035 : .96,
              opacity: 0,
            }, {
              x: 0,
              y: 0,
              ...
              scale: 1,
              opacity: 1,
              duration: desktop ? .9 + (index % 3) * .1 : .72,
              ease: "power3.out",
          ...
          const imageDrifts = [-9, 7, -6, 10, -8, 6];
          cardImages.forEach((image, index) => {
            const drift = mobile ? imageDrifts[index % imageDrifts.length] * .28 : imageDrifts[index % imageDrifts.length];
            gsap.fromTo(image, { yPercent: -drift, scale: 1.1 }, {
              yPercent: drift,
              scale: desktop ? 1.16 + (index % 3) * .025 : 1.11,
              ease: "none",
              scrollTrigger: { trigger: image.closest(".event"), start: "top bottom", end: "bottom top", scrub: mobile ? .3 : .18 + (index % 3) * .06 },
            });
          });
